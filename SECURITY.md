...
- **Waking an agent never widens it.** `send-to-codex` reloads an agent the Codex daemon unloaded only if `spawn-peer` created it. It uses the first line `spawn-peer` recorded for it, never a later one. The record must not be writable by other users, the folder must not be a protected one, and the daemon's own record must agree: same folder, and not a thread a person started. Menu Resume passes the recorded settings too, but only while the record is private to you and the folder is not a protected one. Otherwise, and where the record is silent, it passes the strictest ones. It does not ask the daemon to agree, because you started the resume yourself.
...
- **Private logs.** Logs are created 0600, in 0700 folders when the tools create the folder, and a log left readable by others is tightened when it is next written.
