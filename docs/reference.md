...
- **Log line:** `time_utc`, `channel` (`claude socket`), `sender`, `thread`, `thread_uuid`, `recipient_name`, `first_line`, `result` (`written` or `error`), `queued_id` (always null), `msg_id`, `reply_types`, `error` and `exit_code`. Written means the socket accepted the bytes and the receiver sent no error. It doesn't mean the session read them.
...
- **Read-only Claude agents wait for you.** A read-only Claude agent runs in `plan` mode, so it stops and asks you before it acts, and that includes sending a reply. That pause is a safety feature, and these tools do not work around it.
...
- **Permanent deletion** (the daemon's `thread/delete` for Codex, `claude rm` for Claude) needs `--delete` and `--approval`. `claude rm` also deletes the session's git worktree if it has one, so check that first. The tool never passes `--discard-unpushed` or `--force-remove-worktree`.
...
- **spawn-peer exit status:** 0 created and recorded. 1 created but not recorded, and the line is printed. 2 usage error, including a name, sender or message that is not UTF-8. 3 refused, nothing created. 4 the create step failed, or the daemon answered in a shape these tools do not know. An agent may still exist after a 4: a Claude session that never appeared in the listing is recorded with `listed=false`, and a Codex thread whose naming failed is archived at once, so check `log/spawned.jsonl`. 5 created and recorded, but the first message was not delivered.
...
- **Quit agent** archives a Codex thread or stops a Claude background session, keeping its conversation. It takes two deliberate steps: `x` arms it, then `y` or Enter confirms, at least 0.6 seconds later, as the very next key, with no other key arriving behind it. A confirm that comes too quickly says so in the bottom line. Quit is not offered for anything running in a pane, which you quit in that pane, or for an interactive Claude session. For an agent `spawn-peer` created, Quit goes through `retire-peer`, so the record notes it.
- **Resume** (`r`) brings a quit agent back in a new window: a Codex thread is unarchived and opened with `codex resume`, and a Claude session is attached. For an agent `spawn-peer` created, `codex resume` gets the recorded sandbox, approvals and folder, never your `config.toml` defaults. It gets the strictest ones where the record is silent, open to other users, or names a protected folder.
- **Instructions** are one line of plain text. Line breaks and control characters are refused. For an agent in a pane the text is typed into that pane. For a Codex agent with no pane it is queued with the same wake and delivery check as `send-to-codex`, waiting up to 3 seconds, and the popup says whether a turn took it. It is your own input, so it carries no teammate header and no `Msg-ID`, has no receipt to track, and is logged only in `log/menu-actions.jsonl`. A Claude session with no pane takes input only in its own terminal, so the popup offers Attach instead.
...
- **Look-only mode:** `AGENT_MENU_LOOK_ONLY` set to any value keeps Open pane, Attach and Resume, and offers no instruction line and no Quit. The sidebar passes this and its other settings to every popup it opens.
...
| `claude --bg`, `claude agents --json`, `claude attach`, `claude stop`, `claude rm`, `claude -p --safe-mode --no-session-persistence --setting-sources` | Documented Claude Code CLI | `claude --help` |
