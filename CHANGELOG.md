...
- Hardening before release, after an independent review: private logs, no settings or hooks in the menu's own `claude` call, a wake that no line written later can widen, clean handling of torn logs, long messages and interrupts, and exit codes that mean one thing each.
