...
class AgentMenuTest(unittest.TestCase):
    ...
    def test_a_codex_thread_that_reports_it_waits_on_the_owner_says_so_without_a_pane(self):
        ...
        self.assertTrue(text.endswith("buttons: Send, Quit agent\n"), text)   # queueing answers no prompt
    ...
    def test_a_finished_spawned_claude_agent_stays_under_its_parent(self):
        ...
        self.assertEqual(self.buttons(f"claude:{CLAUDE_B}"), "buttons: Resume, Quit agent")
        # Retired, it keeps its place for a while like any retired child, with only Resume left, and no
        # QUIT group is drawn while the owner has not asked for it with `a`.
        spawn_log = self.m.dir / "spawned.jsonl"
        before = spawn_log.read_text()
        self.m.log("spawned.jsonl", {"event": "retired", "kind": "claude", "id": "bbbbbbbb", "action": "stop",
                                     "time_utc": stamp(1)})
        lines = self.tree()
        self.assertTrue(any(l.startswith("   └ review") for l in lines), lines)
        self.assertFalse(any("QUIT" in l for l in lines), lines)
        self.assertEqual(self.buttons(f"claude:{CLAUDE_B}"), "buttons: Resume")
        spawn_log.write_text(before)
        self.m.log("spawned.jsonl", {"event": "retired", "kind": "claude", "id": "bbbbbbbb", "action": "stop",
                                     "time_utc": stamp(11)})
        lines = self.tree()
        self.assertFalse(any("review" in l or "QUIT" in l for l in lines), lines)
    ...
    def popup_session(self, key, keys, wait_for, lines=30, columns=98):
        """Run the real popup on a pty, wait until it shows `wait_for`, then type `keys`, a list of
        (bytes, pause after), and end with q. Returns what the popup drew."""
        env = dict(self.m.env, TERM="xterm-256color", LINES=str(lines), COLUMNS=str(columns))
        pid, fd = pty.fork()
        if pid == 0:
            os.execve(sys.executable, [sys.executable, str(BIN / "agent-menu-detail"), key], env)
        shown = bytearray()
        ...
        try:
            ...
            while wait_for not in shown and time.monotonic() < deadline:
                time.sleep(0.05)
            self.assertIn(wait_for, shown)
            for typed, pause in [*keys, (b"q", 0.0)]:
                os.write(fd, typed)
                time.sleep(pause)
    ...
    def test_a_popup_too_small_to_show_its_buttons_acts_on_no_key(self):
        ...
        keys that quit an agent in a full-size popup, x and then Enter, must do nothing there."""
        ...
        self.popup_session(f"codex:{CODEX_C}", [(b"x", 0.8), (b"\r", 1.0)], b"too small", lines=8)
    ...
    def test_a_confirm_with_keys_right_behind_it_is_cancelled(self):
        ...
        typed by hand, with nothing behind the `y`, do quit the agent."""
        self.codex_thread(CODEX_C, "kid")
        self.popup_session(f"codex:{CODEX_C}", [(b"x", 0.8), (b"yes please", 1.0)], b"Quit agent")
        self.assertNotIn("thread/archive", self.daemon.methods())
        self.assertEqual(self.m.actions(), [])
        self.popup_session(f"codex:{CODEX_C}", [(b"x", 0.8), (b"y", 1.5)], b"Quit agent")
        self.assertEqual(self.daemon.params("thread/archive"), [{"threadId": CODEX_C}])
