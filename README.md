...
- **What you can do in the popup:** `Open pane` jumps to the agent's pane. `Attach` opens a background Claude session in a new window. The instruction line sends your words to the agent. `Retire` archives or stops an agent that `spawn-peer` created. It is the one risky action, so it takes two deliberate steps: `r` arms it, then `y` or Enter confirms. The confirm must be the very next key, must come at least half a second later, and must have no other key arriving behind it. Anything else cancels, including pressing `r` again. These actions exist only inside the popup and have no command-line form. That guards against accidents. It is not a security boundary: a program running as you could import the library, just as it could run `tmux send-keys` itself.
- **Your instruction is your own input.** It is one line of plain text: line breaks and control characters are refused, so a pasted block cannot turn into several commands. A paste can fill the instruction line but cannot send it: a pasted Enter never sends, and once any of the text was pasted, the Enter that sends it must come a moment after the last key, which is what you do anyway when you read what you pasted.

  Outside the instruction line a paste cannot act either. Ordinary words hold the hotkeys, so a character arriving within a twentieth of a second of the one before it is treated as pasted: it presses nothing, and it cancels an armed Retire. That last part is what stops a paste that arrives in chunks, which is how a terminal delivers a long paste over a slow link. The pause between chunks looks like a person, but the characters inside a chunk cancel the button long before a later chunk can reach a `y`. One shape is left: a chunk that is exactly `r`, a pause, then a chunk starting with `y`, which is the same input a person makes. The popup catches that one by looking for keys still arriving behind the confirm and cancelling when it finds any. For an agent in a pane it is typed into that pane. For a Codex agent with no pane it is queued as plain input with no teammate header. A Claude session with no pane takes input only in its own terminal, so the popup offers `Attach` instead.
- **The menu only types into a pane it is sure about.** The pane must be running that agent's own CLI. A live Claude session matches only by process: the menu walks up from the session's own process to the first tmux pane. If another session lies in between, that pane belongs to the other one, so this session gets no pane at all. This is what stops an agent that was started by another agent's Bash tool from inheriting its parent's pane. A session whose process runs under no pane gets no pane either, even when a pane's title matches its name.  Otherwise a session matches by its name in the pane title. A Codex thread matches the name part of its title, never the project part. A title can be read more than one way: a name may itself contain `|`, and the CLIs put a status glyph in front while busy. The menu considers every reading, and then one agent with two candidate panes gets no pane, and one pane claimed by two agents goes to neither. A thread that is not running claims nothing. Right before typing, the menu checks again that the pane still has the same first process, still runs that CLI in the foreground, still shows that session's name, and, for a Claude session, that the session's own process still runs under that pane. When a CLI exits its shell keeps the pane, and you may have started another session in it under the same name.
