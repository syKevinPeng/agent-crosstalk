...
- **What you can do in the popup:** `Open pane` jumps to the agent's pane. `Attach` opens a background Claude session in a new window. The instruction line sends your words to the agent. `Retire` archives or stops an agent that `spawn-peer` created, and asks twice. These actions exist only inside the popup and have no command-line form. That guards against accidents. It is not a security boundary: a program running as you could import the library, just as it could run `tmux send-keys` itself.
- **Your instruction is your own input.** It is one line of plain text: line breaks and control characters are refused, so a paste cannot turn into several commands. For an agent in a pane it is typed into that pane. For a Codex agent with no pane it is queued as plain input with no teammate header. A Claude session with no pane takes input only in its own terminal, so the popup offers `Attach` instead.
- **The menu only types into a pane it is sure about.** The pane must be running that agent's own CLI. A Claude session matches by process id or by its name in the pane title. A Codex thread matches only the name part of its title, never the project part. One agent with two candidate panes gets no pane, and one pane claimed by two agents goes to neither. A thread that is not running claims nothing. Right before typing, the menu checks again that the pane still has the same first process and still runs that CLI in the foreground, because when a CLI exits its shell keeps the pane.
- **Names are untrusted text.** Other agents choose their session names. Control characters and escape sequences in a name are replaced before it reaches the screen. A name is escaped before tmux sees it, because tmux would otherwise expand `#{...}` and run `#(command)` found in a popup title or window name.
